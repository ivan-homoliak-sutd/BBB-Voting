...
contract LocalCrypto {
  ...
  function commitToVote(uint[4] memory params, uint[2] memory xG, uint[2] memory yG, uint[2] memory y, uint[2] memory a1, uint[2] memory b1,
        uint[2] memory a2, uint[2] memory b2)
  ...
  function createZKP(uint x, uint v, uint[2] memory xG) public view returns (uint[4] memory res) {
      ...
      // Get g^{v}
      uint[3] memory vG = Secp256k1._mul(v, G1);
  ...
  function verifyZKP(uint[2] memory xG, uint r, uint[3] memory vG) public view returns (bool){
      ...
      // Get g^{r}, and g^{xc}
      uint[3] memory rG = Secp256k1._mul(r, G1);
  ...
  // random 'w', 'r1', 'd1'
  function create1outof2ZKPNoVote(uint[2] memory xG, uint[2] memory yG, uint w, uint r2, uint d2, uint x)
    public view
    returns (uint[10] memory res, uint[4] memory res2)
  {
  ...
  // random 'w', 'r1', 'd1'
  // TODO: Make constant
  function create1outof2ZKPYesVote(uint[2] memory xG, uint[2] memory yG, uint w, uint r1, uint d1, uint x)
    public view
    returns (uint[10] memory res, uint[4] memory res2)
  {
      // y = h^{x} * g
  ...
  function verify1outof2ZKP(uint[4] memory params, uint[2] memory xG, uint[2] memory yG, uint[2] memory y, uint[2] memory a1, uint[2] memory b1,
     uint[2] memory a2, uint[2] memory b2)
     public view
     returns (bool)
  {
    ...
    // return t,n.
    function createEqualityProof(uint r1, uint r2, uint r3, uint[2] memory c1, uint[2] memory c2) public view returns (uint[2] memory t, uint n) {
    ...
    function computeFirstHalfEquality(uint[2] memory c1, uint[2] memory c2, uint h, uint[2] memory t) public pure returns (uint[2] memory left){
    ...
    function createInequalityProof(uint b1, uint b2, uint r1, uint r2, uint r3, uint r4, uint[2] memory c1, uint[2] memory c2)
        public view
        returns (uint[2] memory t1, uint[2] memory t2, uint n1, uint n2)
    {
    ...
    function computeSecondHalfInequality(uint[2] memory c1, uint[2] memory c2, uint[2] memory t2, uint h)
        private pure
        returns (uint[3] memory right)
    {
      uint[3] memory negative_c2;
      // Negate the 'y' co-ordinate of C2
      negative_c2[0] = c2[0];
    ...
    function verifyInequalityProof(uint[2] memory c1, uint[2] memory c2, uint[2] memory t1, uint[2] memory t2, uint n1, uint n2) public view returns (bool) {
      if(!Secp256k1.isPubKey(c1)) { revert(); }
      if(!Secp256k1.isPubKey(c2)) { revert(); }
