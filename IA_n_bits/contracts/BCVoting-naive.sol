...
/**
    ...
    inStage(StageEnum.PRE_VOTING)
  {
    ...
    for (uint i = 0; i < ephemeralPKs.length; i++) {
      ...
      BN.instance memory numerator =  BN.instance(abi.encodePacked(uint(0x01)), false, 1);
      for (uint j = 0; j < i; j++) {
        numerator = BN.modmul(numerator, ephemeralPKs[j], P);
      }

      BN.instance memory denominator = BN.instance(abi.encodePacked(uint(0x01)), false, 1);
      for (uint j = i + 1; j < ephemeralPKs.length; j++) {
        denominator = BN.modmul(denominator, ephemeralPKs[j], P);
      }

      BN.instance memory tmpRes = BN.instance(inv_result[i], false, inv_result_len[i]);
      MpcPKs.push(BN.modmul(numerator, BN.mod_inverse(denominator, P, tmpRes), P));
    }
