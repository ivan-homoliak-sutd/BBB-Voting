...
Object.defineProperty(Voter.prototype, 'cache_modinv_MPC', {
  get: function () {
    return this._cache_modinv_MPC;
  }
})
...
Voter.prototype.computeMpcPK = function(ephemeralPKs){
  ...
  this._cache_modinv_MPC = utils.inv_mod(denominator, this._P);
  this._h =  (numerator * this._cache_modinv_MPC) % this._P;
  console.log(`\t The voter[${this._id}] computed his MPC key (i.e. h) =  ${this._h.toString(16)}`);
}
...
Voter.prototype.getBlindedVote = function(){
    ...
    let w = BigInt(utils.randomBytes(this._P + 10n));

    for (let i = 0; i < this.candidates.length; i++) {
        // console.log(`\t Processing candidate[${i}]...`)
        if(i != this._vote){
            // TODO: ensure that MSB of the following is 1
    ...
    for (let i = 0; i < this._a.length; i++) {
      inputForHash.push(utils.toPaddedHex(this._a[i].toString(16), this.sizeP));
      assert(this._a[i] < this._P);
    }
    for (let i = 0; i < this._b.length; i++) {
      inputForHash.push(utils.toPaddedHex(this._b[i].toString(16), this.sizeP));
      assert(this._b[i] < this._P);
    }
    ...
    console.log(`\t\t c % (P - 1) = ${(BigInt(this._c) % (this._P - 1n)).toString(16)}`);

    // Adjust d and r for the candidate that the voter vote for
    this._d[this._vote] = (BigInt(this._c) - utils.sumBI(this._d, this._P - 1n)) % (this._P - 1n);

    console.log("\t\t Sum of all ds is:", utils.sumBI(this._d, this._P - 1n).toString(16));
    ...
    return [
        ...utils.BI2BNarraySolidity(this._a),
        ...utils.BI2BNarraySolidity(this._b),
        ...utils.BI2BNarraySolidity(this._r),
        ...utils.BI2BNarraySolidity(this._d),
        ...utils.BI2BNsolidity(this._bvote),
    ];
}
