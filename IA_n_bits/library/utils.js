let W3 = require('web3');
const provider = new W3.providers.HttpProvider('http://localhost:8777')
...
class Utils {
    ...
    BIarrayToHex = function (arr, paddingBytes) {
        // usefull for up to 256 bit modulus
    ...
    shaX = function(a, newSizeB) {
        ...
        // console.log("a = ", a);
        var retHash = (Array.isArray(a)) ?
                W3.utils.soliditySha3(...a) :
                W3.utils.soliditySha3(a).substring(0, 2 + 2 * newSizeB);

        if(stdHashSizeB < newSizeB){
            // if modulus is bigger, we also need to accomodate the size of hash
            let tmpHash = (' ' + retHash).slice(1); // toPaddedHex
            while (retHash.length < 2 + (newSizeB * 2)) {
                // console.log("tmpHash = ", tmpHash);
                tmpHash = W3.utils.soliditySha3(tmpHash);
                retHash += tmpHash.substring(2)
            }
            if(retHash.length > newSizeB){
                retHash = retHash.substring(0, 2 + 2 * newSizeB);
            }
        }
        return retHash;
    }

    alignHexto32B = function(a) {
        if(typeof a != "string" || a.substr(0, 2) == '0x'){
            // console.log(`alignHexto32B: a = ${a} | len = ${a.length}`);
            throw new Error(`alignHexto32B: a = ${a} | len = ${a.length}`);
        }
        if(a.length % 2 != 0){
            a = "0" + a;
        }
        ...
        let lenB =  ((a.length) / 2);
        let overflowingB = lenB % EVM_WORD_BYTES;
        if(overflowingB != 0){
            return "0x" + '0'.repeat((EVM_WORD_BYTES - overflowingB) * 2) + a; // shift data on the right and align them to 32B
        }
        return "0x" +  a;
    }

    BI2BNarraySolidity = function (arr, getLen = false) {
        assert(Array.isArray(arr));
        let retVal = [];
        let retLen = [];
        arr.forEach(e => {
            assert(typeof e == "bigint");
            let asBN = this.BI2BNsolidity(e, true);
            retVal.push(asBN[0]);
            retLen.push(asBN[1]);
        });

        if(getLen)
            return [retVal, retLen];

        return [retVal];
    }

    BI2BNsolidity = function(a, getLen = false) {
        if(! typeof a == "bigint"){
            throw new Error("BI2BNsolidity type must be bigint");
        }

        let aBN = new BN(a.toString(16), 16);
        let a_val = this.alignHexto32B(a.toString(16));
        // let a_val = "0x" + a.toString(16);
        let a_bitlen = aBN.bitLength();

        // let abiEncoded = web3.eth.abi.encodeParameter(
        //     {
        //         "instance": {
        //             "val": 'bytes',
        //             "neg": 'bool',
        ...
        // console.log(`a_val = ${a_val}, a_bitlen = ${a_bitlen}`);
        // return [abiEncoded, a_val, a_bitlen];
        if(getLen)
            return [a_val, a_bitlen];
        else
            return [a_val];
    }
